...
if ( ! class_exists( 'Deon_Rest_API' ) ) {
	...
	class Deon_Rest_API {
		...
		public function __construct() {
			// Init variables
			$this->set_namespace( 'deon/v1' );

			// Localize theme's main js script with rest variable
			add_filter( 'deon_filter_localize_main_js', array( $this, 'localize_script' ) );
		...
		function localize_script( $global ) {
			...
			return apply_filters( 'deon_filter_rest_api_global_variables', $global, $this->get_namespace() );
		}

		function register_rest_api_route() {
			$routes = apply_filters( 'deon_filter_rest_api_routes', array() );
