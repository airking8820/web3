...
if ( ! function_exists( 'deon_add_rest_api_pagination_global_variables' ) ) {
	/**
	 ...
	 * @return array
	 */
	function deon_add_rest_api_pagination_global_variables( $global, $namespace ) {
		$global['paginationRestRoute'] = $namespace . '/get-posts';

		return $global;
	}

	add_filter( 'deon_filter_rest_api_global_variables', 'deon_add_rest_api_pagination_global_variables', 10, 2 );
}

if ( ! function_exists( 'deon_add_rest_api_pagination_route' ) ) {
	/**
	 ...
	 * @return array
	 */
	function deon_add_rest_api_pagination_route( $routes ) {
		$routes['pagination'] = array(
			'route'    => 'get-posts',
			'methods'  => WP_REST_Server::READABLE,
			'callback' => 'deon_get_new_posts',
			'args'     => array(
				'options' => array(
					'required'          => true,
					...
					'description'       => esc_html__( 'Options data is array with all selected shortcode parameters value', 'deon' ),
				),
			),
		);
	...
	add_filter( 'deon_filter_rest_api_routes', 'deon_add_rest_api_pagination_route' );
}

if ( ! function_exists( 'deon_get_new_posts' ) ) {
	/**
	 * Function that load new posts for pagination functionality
	 *
	 * @return void
	 */
	function deon_get_new_posts() {

		if ( ! isset( $_GET ) || empty( $_GET ) ) {
			deon_get_ajax_status( 'error', esc_html__( 'Get method is invalid', 'deon' ) );
		} else {
			...
			if ( ! empty( $options ) ) {
				...
				$query_args = deon_get_query_params( $options );
				...
				echo apply_filters( "deon_filter_{$get_template_part}", $get_template_part( $module . '/' . $shortcode, 'templates/loop', '', $options ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				...
				if ( 'standard' === $options['pagination_type'] ) {
					$pagination_html = apply_filters( "deon_filter_{$get_template_part}_pagination", deon_get_template_part( 'pagination', 'templates/pagination', $options['pagination_type'], $options ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				}

				$data = apply_filters(
					'deon_filter_pagination_data_return',
					array(
						'html'            => $html,
				...
				deon_get_ajax_status( 'success', esc_html__( 'Items are loaded', 'deon' ), $data );
			} else {
				deon_get_ajax_status( 'error', esc_html__( 'Options are invalid', 'deon' ) );
			}
		}
	}
}

if ( ! function_exists( 'deon_get_query_params' ) ) {
	/**
	 ...
	 * @return array
	 */
	function deon_get_query_params( $params ) {
		...
		return apply_filters( 'deon_filter_query_params', $args, $params );
	}
}

if ( ! function_exists( 'deon_get_pagination_data' ) ) {
	/**
	 ...
	 * @return array
	 */
	function deon_get_pagination_data( $plugin, $module, $shortcode, $post_type, $params ) {
		$data = array();
		...
		if ( ! empty( $post_type ) && ! empty( $params ) ) {
			...
			if ( isset( $params['space'] ) && ! empty( $params['space'] ) ) {
				$params['space_value'] = deon_get_space_value( $params['space'] );
			}

			$data = json_encode( array_filter( array_merge( $additional_params, $params ), 'deon_validate_pagination_data' ) );
		}
...
if ( ! function_exists( 'deon_validate_pagination_data' ) ) {
	/**
	 ...
	 * @return mixed
	 */
	function deon_validate_pagination_data( $value ) {
...
if ( ! function_exists( 'deon_add_link_pages_after_content' ) ) {
	/**
	 * Function which add pagination for blog single and page
	 */
	function deon_add_link_pages_after_content() {

		$args_pages = array(
			'before'      => '<div class="qodef-single-links qodef-m"><span class="qodef-m-single-links-title">' . esc_html__( 'Pages: ', 'deon' ) . '</span>',
			'after'       => '</div>',
			'link_before' => '<span>',
			'link_after'  => '</span>',
			'pagelink'    => '%',
	...
	add_action( 'deon_action_after_blog_single_content', 'deon_add_link_pages_after_content' );
	add_action( 'deon_action_after_page_content', 'deon_add_link_pages_after_content' );
}
